<template>
    <div v-if="authenticated" class="dropdown mx-2">
...
export default {
        ...
        methods: {
            ...
            clearAllNotifications() {
                if (confirm("You want to clear all notifications!")) {
                    this.$store.dispatch('clearAllNotifications')
                    .then((response) => {
                        this.setFlashMessage({ message: 'Notifications cleared !!', type: 'alert-success' });
                    });
            }
            },
            markNotificationAsRead(unread) {
                if (confirm("You want to mark notification as read!")) {
                    this.$store.dispatch('markNotificationAsRead', unread)
                    .then((response) => {
                        this.setFlashMessage({ message: 'Notification readed !!', type: 'alert-success' });
                    })
                }
